...
// ==========================================================
// Projeto preparado para ESP32-C3 no Wokwi e para adaptacao
// posterior ao ESP32-C3 fisico.
...
// Valores tipicos para o microservo simulado no Wokwi.
...
// true: simulacao Wokwi com rede Wokwi-GUEST e acesso por localhost:8180.
// false: placa fisica cria um Access Point local Controle_PWM_ESP32.
const bool MODO_SIMULACAO_WOKWI = true;
...
<body>
  ...
  <script>
    ...
    async function aplicarLed() {
      ...
      const frequencia = limitar(document.getElementById("frequenciaLedNumero").value, 1, 40000);
...
bool iniciarWifi() {
  if (MODO_SIMULACAO_WOKWI) {
    Serial.println("Conectando ao Wi-Fi virtual Wokwi-GUEST...");
    WiFi.mode(WIFI_STA);
    WiFi.begin(WOKWI_SSID, WOKWI_PASSWORD, 6);
    ...
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("ERRO: nao foi possivel conectar ao Wokwi-GUEST.");
      return false;
    }

    Serial.print("IP virtual recebido pelo ESP32: ");
    Serial.println(WiFi.localIP());
    Serial.println("No computador, abra: http://localhost:8180");
    return true;
  }
  ...
  Serial.print("Abra no navegador conectado a essa rede: http://");
