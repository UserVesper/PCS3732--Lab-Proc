...
// multiplicacao e divisao caibam com seguranca em variaveis int64_t.
static_assert(
...
<body>
  ...
  <div class="card">
    ...
    <div class="buttons">
      ...
      <button id="btnDivisao">DIVISAO</button>
  ...
  <div class="card">
    ...
    <p>
      Resto da divisao:
      <code id="restoDivisao">-</code>
    </p>

    <p>
      Resto da divisao em complemento de dois:
      <code id="restoDivisaoBinario">-</code>
    </p>
    ...
    <p>
      ...
      <code id="tempoOperacaoMicros">-</code>
      microssegundos
    </p>
  </div>
  ...
  <script>
    ...
    async function enviarOperacao(operacao) {
      ...
      try {
        ...
        document
          .getElementById("restoDivisao")
          .textContent =
            dados.possuiRestoDivisao
              ? dados.restoDivisao
              : "-";

        document
          .getElementById("restoDivisaoBinario")
          .textContent =
            dados.possuiRestoDivisao
              ? dados.restoDivisaoBinario
              : "-";
        ...
        document
          .getElementById("tempoOperacaoMicros")
          .textContent = dados.tempoOperacaoMicros;
      } catch (erro) {
        atualizarStatus(
          "Erro: " + erro.message,
          true
...
// Divisao inteira binaria pelo algoritmo restaurador.
//
// O operador / nao e utilizado para realizar a operacao.
...
bool dividir(
  ...
  if (divisor == 0) {
    mensagemErro =
      "Nao e possivel realizar divisao por zero.";

    return false;
  }

  bool quocienteNegativo =
    (dividendo < 0) != (divisor < 0);

  bool restoNegativo =
    dividendo < 0;

  uint64_t dividendoPositivo =
    dividendo < 0
      ? static_cast<uint64_t>(-dividendo)
      : static_cast<uint64_t>(dividendo);

  uint64_t divisorPositivo =
    divisor < 0
      ? static_cast<uint64_t>(-divisor)
      : static_cast<uint64_t>(divisor);

  uint64_t quocientePositivo = 0;
  uint64_t restoPositivo = 0;
  ...
  for (
    ...
    restoPositivo |=
      (dividendoPositivo >> bit) & 1ULL;

    if (restoPositivo >= divisorPositivo) {
      restoPositivo -= divisorPositivo;
  ...
  quociente =
    static_cast<int64_t>(quocientePositivo);

  resto =
    static_cast<int64_t>(restoPositivo);
...
bool calcular(
  ...
  int64_t valorB,
  int64_t& resultado,
  int64_t& restoDivisao,
  bool& possuiRestoDivisao,
  String& mensagemErro
) {
  possuiRestoDivisao = false;
  restoDivisao = 0;
  if (operacao == "add") {
...
void responderCalculo() {
  ...
  int64_t resultado = 0;
  int64_t restoDivisao = 0;
  bool possuiRestoDivisao = false;
  String mensagemErro = "";
  ...
  json += "\"restoDivisao\":";
  json += int64ParaString(restoDivisao);
  json += ",";

  json += "\"restoDivisaoBinario\":\"";
  json += decimalParaBinarioComplementoDeDois(restoDivisao);
  json += "\",";
  ...
  json += "\"tempoOperacaoMicros\":";
  json += String(tempoOperacaoMicros);
  ...
  if (possuiRestoDivisao) {
    Serial.print(
      "Resto da divisao: "
    );
    Serial.println(
      int64ParaString(restoDivisao)
    );
    ...
    Serial.println(
      decimalParaBinarioComplementoDeDois(restoDivisao)
    );
  }
